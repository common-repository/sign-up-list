...
/**
 * This class extends WP_List_Table for displaying and managing the entries on the sign-up list.
 * 
...
class Entries_List_Table extends \WP_List_Table{
    ...
    function get_columns()
    {
        $columns = array(
                ...
                'firstname' => __( 'First name', 'sign-up-list' ),
                'lastname'  => __( 'Last name', 'sign-up-list' ),
                'email'     => __( 'Email address', 'sign-up-list')
        );
        if ( ! empty ( get_option( 'sul_general_admin' )['extra_label'] ) ) {
            $columns +=  ['extra_1' => get_option( 'sul_general_admin' )['extra_label'] ];
        }
        $columns += ['created'   => __( 'Timestamp', 'sign-up-list' ) ];
        return $columns;
    }
    ...
    function prepare_items()
    {
        if ( isset( $_POST['s'] ) ) {
            ...
            $this->table_data = Database::get_entries( $search );
        } else {
            $this->table_data = Database::get_entries();
        }
    ...
    protected function get_sortable_columns()
    {
      ...
      if ( ! empty ( get_option( 'sul_general_admin' )['extra_label'] ) ) {
            $sortable_columns += ['extra_1' => array('extra_1', false) ];
      }  
    ...
    /**  
     * Adding action links to column firstname.
     * 
    ...
    /**  
     * Adding action links to column lastname.
     * 
     * @param array $item. The table row.
     * 
     * @return string The characters to echo for the column value with action links attached.
    ...
    {
        $actions = $this->get_actions( $item );
        return sprintf('%1$s %2$s', $item['lastname'], $this->row_actions($actions));
    }
        ...
        switch ( $action ) {
            ...
            case 'delete_selected':
                if ( isset( $_POST['element'] ) ) {
                    Admin::delete_entries( $_POST['element'] );
                    return true;
                } else {
                    return false;
                }
